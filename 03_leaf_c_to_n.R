...
VEGmeans2 <-rename(VEGmeans2, c("REMAINper.x" = "mean_per"))
VEGmeans2 <-rename(VEGmeans2, c("REMAINper.y" = "sd"))
...
ELEVmeans2 <-rename(ELEVmeans2, c("REMAINper.x" = "mean_per"))
ELEVmeans2 <-rename(ELEVmeans2, c("REMAINper.y" = "sd"))
...
# ggplot(elev, aes(x=days, y=mean_per * 100, group=ELEV, colour=ELEV))+
#   geom_line(aes(linetype=ELEV), size=1)+
#   geom_point(size=3, fill="white")+
#   scale_colour_manual(values=cbPalette)+
#   scale_y_continuous(limits= c(0,100), expand=c(0,0))+
#   geom_errorbar(aes(ymin=(mean_per-SE) * 100, ymax=(mean_per+SE) * 100, width=.1))+
#   theme_bw()+
...
VEGmeans <-merge(VEGmeans, VEGmeansSD, by=c("VEG", "COLLECT") )

VEGmeans <-merge(VEGmeans, VEGmeansN, by=c("VEG", "COLLECT") )
VEGmeans <-rename(VEGmeans, c("REMAINper.x" = "mean_per"))
VEGmeans <-rename(VEGmeans, c("REMAINper.y" = "sd"))
VEGmeans <-rename(VEGmeans, c("REMAINper" = "n"))


pN <- ggplot(leavesx, aes(x=days, y=newN, color=ELEV))+
  geom_point()
pN

#bringing in lignin
lignin <-  read.csv(file="lignin_cvi_firstrun.csv", head=TRUE, strip.white=TRUE)

str(lignin)

#calculating percentage of lignin in sample based on coeff from Fukushima $ Hatfield, 2004 and formula from Pauly article
lignin$lig <- ( (lignin$abs*10) /15) * (150 / lignin$lignin_mass)

#merging
leavesxx <- merge(leavesx, lignin[, c("ID", "lig")], by = "ID")
leavesx$VEG <- factor(leavesx$VEG, levels=c("OPEN", "CANOPY", "SHRUB"))
leavesx$ELEV <- factor(leavesx$ELEV, levels=c("LOW", "MID", "HIGH"))
leavesxx$VEG <- factor(leavesxx$VEG, levels=c("OPEN", "CANOPY", "SHRUB"))
leavesxx$ELEV <- factor(leavesxx$ELEV, levels=c("LOW", "MID", "HIGH"))

leavesxx$ligN <- leavesxx$lig / leavesxx$N

###lignin plot
pN_to_lignin <- ggplot(data=leavesxx, aes(x = COLLECT, y=ligN, color=VEG))+
  geom_point(size=6)+
  theme_fivethirtyeight2()+
...
pelevN_to_lignin <- ggplot(data=leavesxx, aes(x = days, y=ligN, color=ELEV))+
  geom_point(size=6)+
  theme_fivethirtyeight2()+
  scale_color_manual(values = darjeelingmod)+
  theme(axis.title.x = element_text(size=18),
...
#keep this plot
p.c <- ggplot(leavesx, aes(x=days, y = C))+
  geom_point(size=4)+
  ylab("C (%)")+
  xlab("Days")+
  theme(axis.title.x = element_text(size=18),
...
p.c + facet_grid(ELEV~VEG)+
  theme(strip.text.x = element_text(size=12),
        strip.text.y = element_text(size=12),
        strip.background = element_rect(colour="#FFFFFF", fill="#FFFFFF"))

p.n <- ggplot(leavesx, aes(x=days, y = N))+
  geom_point(size=4)+
  ylab("N (%)")+
  xlab("Days")+
  theme(axis.title.x = element_text(size=18),
        ...
        axis.text.x = element_text(size =18))+
  stat_smooth(method=loess, se=FALSE, size=1)
p.n + facet_grid(ELEV~VEG)+
  theme(strip.text.x = element_text(size=12),
        strip.text.y = element_text(size=12),
        strip.background = element_rect(colour="#FFFFFF", fill="#FFFFFF"))

p.lig <- ggplot(leavesxx, aes(x=days, y = lig))+
  geom_point(size=4)
p.lig + facet_grid(ELEV~VEG)

p.ligN <- ggplot(leavesxx, aes(x=days, y = ligN))+
  geom_point(size=4)+
  theme(axis.title.x = element_text(size=18),
        axis.title.y = element_text(size=18),
        axis.text.y = element_text(size=18),
        axis.text.x = element_text(size =18))+
  stat_smooth(method=loess, se=FALSE, size=1)
p.ligN + facet_grid(ELEV~VEG)+
  theme(strip.text.x = element_text(size=12),
        strip.text.y = element_text(size=12),
        strip.background = element_rect(colour="#FFFFFF", fill="#FFFFFF"))

#also keep this plot
p.CN <-ggplot(leavesx, aes(x=days, y = CN))+
  geom_point(size=4)+
  theme(axis.title.x = element_text(size=18),
        axis.title.y = element_text(size=18),
        axis.text.y = element_text(size=18),
...
leavesxx <-na.omit(leavesxx)
ddply(leavesxx,~COLLECT,summarise,mean=mean(ligN))
ddply(leavesxx,~VEG,summarise,mean=mean(ligN))
ddply(leavesxx,~COLLECT,summarise,mean=mean(N))
ddply(leavesxx,~VEG,summarise,mean=mean(N))
ddply(leavesxx,~COLLECT,summarise,mean=mean(lig))
ddply(leavesxx,~COLLECT,summarise,mean=mean(lig))


p.lig.decomp <- ggplot(leavesxx, aes(x=CN, y = REMAINper))+
  geom_point(size=4)+
  theme(axis.title.x = element_text(size=18),
        axis.title.y = element_text(size=18),
        axis.text.y = element_text(size=18),
...
p.lig.decomp + facet_grid(ELEV~VEG) +
  theme(strip.text.x = element_text(size=12),
        strip.text.y = element_text(size=12))


#######################3
# Finally running stats on C and N from leaf litter

hist(sqrt(leavesx$C))
shapiro.test(sqrt(leaves$))
